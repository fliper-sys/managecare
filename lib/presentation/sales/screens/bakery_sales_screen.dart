...
class BakerySalesScreen extends StatefulWidget {
  final String title;
  final bool enableStoreSwitcher;
  final bool showOwnerProcurementFab;

  const BakerySalesScreen({
    super.key,
    this.title = 'Bakery Sales',
    this.enableStoreSwitcher = false,
    this.showOwnerProcurementFab = false,
  });

  @override
  State<BakerySalesScreen> createState() => _BakerySalesScreenState();
}

class _BakerySalesScreenState extends State<BakerySalesScreen>
  ...
  Future<void> _showDistributorSaleDialog(BuildContext context, Product product) async {
    ...
    try {
      ...
      if (!mounted) return;
      try {
        final printService = ThermalPrintingService();
        await printService.printTextReceipt(
          receiptText: ThermalPrintingService.createCompleteReceipt(
            businessName: context.read<BusinessProvider>().currentBusiness?.name ?? 'Bakery',
            paperWidth: 58,
            items: [
              {
                'name': product.name,
            ...
            orderId: 'DS-${DateTime.now().millisecondsSinceEpoch.toString().substring(6)}',
            cashier: context.read<AuthProvider>().currentUser?.fullName ?? context.read<AuthProvider>().currentUser?.email,
        ...
        // receipt printing is best-effort; don't block the sale workflow
      }
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to record distributor sale: $e'), backgroundColor: Colors.red),
      );
    }
  }
            ...
            child: TabBarView(
              ...
              children: [
                ...
                _ProductsGrid(
                    ...
                    onRecordDistributorSale: (product) => _showDistributorSaleDialog(context, product),
...
class _ProductsGrid extends StatelessWidget {
  ...
  final Function(Product) onRecordDistributorSale;
  ...
  const _ProductsGrid(
      ...
      required this.onRecordDistributorSale,
          ...
          return GridView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return _ProductCard(
                ...
                onDistributorSale: () => onRecordDistributorSale(product),
...
class _ProductCard extends StatelessWidget {
  ...
  const _ProductCard({required this.product, required this.onAdd, this.onDistributorSale, required this.globalPricingMode});
