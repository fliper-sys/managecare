...
class ReportsProvider extends ChangeNotifier {
  ...
  /// Builds a WhatsApp-ready text summary of the currently-loaded sales
  /// report (whatever date range/business the screen last called
  /// generateSalesReport with). sale_items has no cost column of its own
  /// (only unit_price), so gross profit is computed by joining each sold
  /// product against its *current* inventory cost_price via the same
  /// cost-map helper the financial report uses - a sold-then-since-repriced
  /// product's margin here reflects today's cost, not the cost at sale time,
  /// same caveat that already applies to the financial report.
  Future<String> buildWhatsAppSalesSummary(String businessId) async {
    final costMap = await _getInventoryCostMap(businessId);
    ...
    for (final report in _salesReports) {
      ...
      categoryTotals[report.category] =
      ...
      for (final product in report.products) {
        ...
        final costPerUnit = costMap[productId] ?? 0.0;
    ...
    final transactions = _salesReports.length;
    final averageSale = transactions == 0 ? 0.0 : totalRevenue / transactions;
    final dateFormat = DateFormat('MMM d, yyyy');
    final period = _salesStartDate.year == _salesEndDate.year &&
            _salesStartDate.month == _salesEndDate.month &&
            _salesStartDate.day == _salesEndDate.day
        ? dateFormat.format(_salesStartDate)
        : '${dateFormat.format(_salesStartDate)} - ${dateFormat.format(_salesEndDate)}';
    ...
    if (categoryTotals.isNotEmpty) {
      ...
      for (final entry in categoryTotals.entries) {
        buffer.writeln('- ${entry.key}: ${formatCurrency(entry.value)}');
      }
    }
    ...
    buffer
      ..writeln()
      ..write('Generated by Manage Care');
