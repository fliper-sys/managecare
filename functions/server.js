...
function buildAuthResponse(user, accessToken, refreshToken) {
  ...
  return {
    ...
    refresh_token: refreshToken || 'placeholder_refresh_' + user.id,
    user: {
      id: user.id,
      aud: 'authenticated',
      role: 'authenticated',
...
app.post('/auth/v1/token', async (req, res) => {
  ...
  try {
    if (grant_type === 'refresh_token' && refresh_token) {
      const refreshUserId = refresh_token.startsWith('placeholder_refresh_')
        ? refresh_token.substring('placeholder_refresh_'.length)
        : null;
      const result = await pool.query(
        refreshUserId
          ? 'SELECT id, email, full_name, phone_number, created_at, updated_at FROM profiles WHERE id = $1'
          : 'SELECT id, email, full_name, phone_number, created_at, updated_at FROM profiles WHERE lower(email) = $1',
        [refreshUserId || normalizeEmail(req.body.email)]
      );
      ...
      const user = result.rows[0];
      const token = signAccessToken(user);
      return res.json(buildAuthResponse(user, token, refresh_token));
    }
    ...
    const token = signAccessToken(user);
    res.json(buildAuthResponse(user, token, refresh_token));
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});
...
app.post('/rest/v1/rpc/create_business_with_owner', async (req, res) => {
  ...
  const ownerId = p_owner_id || bearerUser?.id;

  if (!p_name || !ownerId) {
    return res.status(400).json({ error: 'p_name and owner id are required' });
  }
...
app.get('/rest/v1/:table', async (req, res) => {
  try {
    ...
    if (wantsSingleObject(req)) {
      return result.rows.length ? res.json(result.rows[0]) : res.status(406).json({ error: 'No rows found' });
    }
    res.json(result.rows);
  } catch (err) {
    res.status(400).json({ error: err.message });
